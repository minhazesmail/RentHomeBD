...
begin
  ...
  if exists (
    select 1
    from public.property_moderation_actions action
    where action.property_id = old.id
  ) then
  ...
  if exists (
    ...
    where report.property_id = old.id
  ) then
  ...
  if exists (
    ...
    where conversation.property_id = old.id
  ) then
...
create trigger properties_guard_hard_delete
before delete on public.properties
for each row execute function private.guard_property_hard_delete();

-- Mirror the trigger rule at RLS level so non-draft deletes are rejected before reaching it.
...
using (
  ...
  and not exists (
    select 1
    from public.property_moderation_actions action
    where action.property_id = id
  )
  and not exists (
    select 1
    from public.listing_reports report
    where report.property_id = id
  )
  and not exists (
    select 1
    from public.conversations conversation
    where conversation.property_id = id
  )
);
