-- Multi-signal recommended search; same RPC signature and public data boundary.
-- See docs/smart-property-search.md for weights, assumptions and rollout.
...
begin
  ...
  -- All hard filters run before scoring. Fixed distance/age scales make ranks
  -- independent of outliers, result count and the chosen map radius.
  scored as (
    select candidates.*,
      0.60 / (1.0 + candidates.distance_meters / 2000.0)
      + case
          when max_rent > coalesce(min_rent, 0) and candidates.rent_bdt is not null
          then 0.25 * greatest(0.0, least(1.0,
            (max_rent - candidates.rent_bdt)::double precision
            / (max_rent - coalesce(min_rent, 0))))
          else 0.0
        end
      + case
          when candidates.last_confirmed_at is not null
          then 0.15 / (1.0 + greatest(0.0,
            extract(epoch from (now() - candidates.last_confirmed_at)) / 604800.0))
          else 0.0
    ...
    select
      ...
      row_number() over (
        order by
          ...
          case when sort_mode = 'recommended' then candidates.recommendation_score end desc,
