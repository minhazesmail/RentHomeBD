...
for (const [file, root] of parsed) {
  ...
  root.walkRules((rule) => {
    ...
    for (const selector of rule.selectors) {
      ...
      // classes scope the rule but do not change ownership of the target class.
  ...
  // Manifests are selector-free by design. Every stylesheet that actually owns
  // selectors must be imported through one named architectural layer.
