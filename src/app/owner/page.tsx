...
export default async function OwnerPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  return (
    <main className="owner-shell owner-management-shell">
      ...
      {params.notice === "saved" && <div className="success-message">Draft saved. You can return to it anytime.</div>}
      {params.notice === "submitted" && <div className="success-message">Listing submitted for moderation. We’ll keep its status visible here.</div>}
