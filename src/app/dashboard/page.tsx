...
export default async function DashboardPage({
  ...
  const [
    ...
    settledData(
      supabase
        .from("profiles")
        .select(
          "phone_verified_at, role_verified_at, role_verified_role, preferred_tenant_type",
        )
        .eq("id", auth.userId)
        .maybeSingle(),
    ),
  ...
  if (canList) {
    const data = await settledData(
      supabase.from("properties").select("status").eq("owner_id", auth.userId),
    );
    ownerProperties = (data ?? []) as OwnerStatusRow[];
  }

  const roleVerified = Boolean(
    trustProfile?.role_verified_at &&
      trustProfile?.role_verified_role === auth.profile.primary_role,
  );
  const preferredTenant = trustProfile?.preferred_tenant_type ?? null;
  const phoneVerified = Boolean(trustProfile?.phone_verified_at);
  const unreadMessageCount = (
    (unreadInbox ?? []) as InboxUnreadRow[]
  ).reduce((sum, row) => sum + Number(row.unread_count ?? 0), 0);
  const liveListingCount = ownerProperties.filter(
    (row) => row.status === "available",
  ).length;
  const listingAttentionCount = ownerProperties.filter(
    (row) =>
      row.status === "pending_confirmation" || row.status === "rejected",
  ).length;
  const reviewListingCount = ownerProperties.filter(
    (row) => row.status === "pending_review",
  ).length;
  const draftListingCount = ownerProperties.filter(
    (row) => row.status === "draft",
  ).length;
  const ownerNeedsTrustAttention = canList && (!phoneVerified || !roleVerified);

  const renterNextAction =
      ...
      : unreadMessageCount > 0
        ...
        : ownerNeedsTrustAttention
          ...
          : {
              ...
              title:
                liveListingCount > 0
                  ? `${liveListingCount} live propert${liveListingCount === 1 ? "y is" : "ies are"} discoverable.`
                  : "Create the next property renters can discover.",
              copy:
                liveListingCount > 0
                  ? "Keep listings fresh and monitor conversations as renters discover them."
                  : "Build a guided listing, add the exact map pin, and submit it for moderation.",
  ...
  return (
    <main
      className={`shell dashboard-shell renter-dashboard-shell${!isRenter ? " owner-dashboard-shell" : ""}`}
    >
      <ProductNavigation authenticated canList={canList} current="dashboard" />
      <section
        className={`dashboard-card renter-dashboard-card${!isRenter ? " owner-dashboard-card" : ""}`}
      >
        <div
          className={`dashboard-header renter-dashboard-header${!isRenter ? " owner-dashboard-header" : ""}`}
        >
          <div>
            <p className="eyebrow">
              {isRenter
            ...
            <h1 className="dashboard-title">
              Welcome
              {auth.profile.display_name ? `, ${auth.profile.display_name}` : ""}.
            </h1>
            <p className="intro">
              {isRenter
                ? "Your next useful rental action is surfaced first."
                : canList
                  ? "See what needs attention across listings, messages, and trust."
        ...
        {params.error === "owner-role-required" && (
          <p className="auth-message">
            Property management is available to owner and agent accounts.
          </p>
        )}
        {params.error === "moderator-role-required" && (
          <p className="auth-message">
            Moderation access is limited to explicitly assigned reviewer accounts.
          </p>
        )}

        {(isRenter || canList) && (
          <section
            className={`dashboard-next-action${!isRenter ? " owner-next-action" : ""}`}
            aria-labelledby="dashboard-next-heading"
          >
            <div className="dashboard-next-icon">
              <NextIcon size={23} aria-hidden="true" />
            </div>
            ...
            <ActionLink href={nextAction.href}>
              {nextAction.label}
        ...
        {isRenter && (
          <>
            <section
              className="renter-account-summary dashboard-metric-grid"
              aria-label="Rental search overview"
            >
              <Link href="/saved" className="dashboard-metric-card">
                <span>Saved homes</span>
                <strong>{savedHomeCount}</strong>
                <p>Shortlist and compare.</p>
              </Link>
              <Link href="/saved" className="dashboard-metric-card">
                <span>Saved searches</span>
                <strong>{savedSearchCount}</strong>
                <p>Reopen map areas.</p>
              </Link>
              <Link
                href="/messages?filter=unread"
                className={`dashboard-metric-card${unreadMessageCount ? " attention" : ""}`}
              >
              ...
              <a
                href="#renter-fit"
                className={`dashboard-metric-card${preferredTenant ? " complete" : " attention"}`}
              >
                <span>Renter fit</span>
                <strong>
                  {preferredTenant
                    ? (tenantLabels[preferredTenant] ??
                      preferredTenant.replaceAll("_", " "))
                    : "Optional"}
                </strong>
                <p>
                  {preferredTenant
                    ? "Used for ranking."
                    : "Add for better ranking."}
                </p>
              </a>
            </section>
            ...
            <section className="renter-dashboard-grid dashboard-secondary-grid">
              <div className="renter-journey-card" id="renter-fit">
                <div className="section-heading">
                  <span>✓</span>
                  <div>
                    <h2>Renter fit preference</h2>
                    <p>
                      {preferredTenant
                        ? "Your current preference is active across map ranking."
                        : "Add a renter type once to personalize compatible-home ranking."}
                    </p>
                  </div>
                </div>
                {preferredTenant ? (
                  <div className="renter-preference-compact">
                    ...
                    <details>
                      <summary>Change preference</summary>
                      <RenterPreferenceForm
                        userId={auth.userId}
                        initialPreference={preferredTenant}
                      />
                    </details>
                  </div>
                ) : (
              ...
              <div className="renter-journey-card dashboard-trust-card">
                <div className="renter-journey-copy">
                  ...
                  <p>
                    {phoneVerified
                      ? "Your phone trust signal is active."
                      : "Verify when you want the phone-verified trust signal or protected contact sharing requires it."}
                  </p>
                </div>
                <div className="renter-journey-actions">
                  <ActionLink variant="text" href="/account/phone">
                    {phoneVerified ? "Manage phone →" : "Verify phone →"}
                  </ActionLink>
                </div>
              </div>
            </section>
        ...
        {!isRenter && canList && (
          <section className="owner-dashboard-workspace owner-dashboard-workspace-redesign">
            <div
              className="owner-dashboard-action-grid owner-dashboard-metrics"
              aria-label="Owner portfolio overview"
            >
              <Link
                className={`owner-dashboard-action-card${listingAttentionCount ? " needs-attention" : ""}`}
                href="/owner?status=attention"
              >
                <span>
                ...
                <div>
                  ...
                  <strong>{listingAttentionCount} listings</strong>
                  <p>Reconfirm or address feedback.</p>
                </div>
                <ArrowRight size={17} />
              </Link>
              <Link
                className="owner-dashboard-action-card"
                href="/owner?status=available"
              >
                <span>
                  <Home size={18} />
                </span>
                <div>
                  ...
                  <strong>{liveListingCount} listings</strong>
                  <p>Discoverable in renter search.</p>
                </div>
                <ArrowRight size={17} />
              </Link>
              <Link
                className="owner-dashboard-action-card"
                href="/owner?status=pending_review"
              >
                <span>
                  <FileCheck2 size={18} />
                </span>
                <div>
                  ...
                  <strong>{reviewListingCount} listings</strong>
              ...
              >
                ...
                <div>
                  <small>Drafts</small>
                  <strong>{draftListingCount} listings</strong>
                  <p>Continue when ready.</p>
                </div>
                <ArrowRight size={17} />
              </Link>
              <Link
                className={`owner-dashboard-action-card${unreadMessageCount ? " needs-attention" : ""}`}
                href="/messages?filter=unread"
              >
                <span>
            ...
            <section
              className="owner-dashboard-status"
              aria-label="Owner trust status"
            >
              <article
                className={
                  ownerNeedsTrustAttention ? "needs-attention" : "is-ready"
                }
              >
                <span className="owner-dashboard-status-icon">
                  <ShieldCheck size={18} />
                </span>
                <div>
                  ...
                  <strong>
                    {ownerNeedsTrustAttention
                      ? "Setup incomplete"
                      : "Phone and role verified"}
                  </strong>
                  <p>
                    {phoneVerified ? "Phone verified" : "Phone not verified"} ·{" "}
                    {roleVerified
                      ? `${auth.profile.primary_role} role verified`
                      : `${auth.profile.primary_role} role awaiting verification`}
                    .
                  </p>
                  {!phoneVerified && (
                    <ActionLink variant="text" href="/account/phone">
        ...
        {!isRenter && !canList && (
          <section className="listing-section dashboard-trust-section">
            <div className="section-heading">
              ...
              <div>
                <h2>Trust status</h2>
                <p>Account verification signals.</p>
              </div>
            </div>
            ...
            <div className="dashboard-actions dashboard-trust-actions">
              <ActionLink variant="secondary" href="/account/phone">
                {phoneVerified ? "Manage verified phone" : "Verify phone"}
              </ActionLink>
            </div>
          </section>
        )}
        ...
        <div className="dashboard-account-tools">
          <div className="dashboard-account-links">
            {!isRenter && !canList && (
              <ActionLink href="/messages">Messages</ActionLink>
            )}
            {!isRenter && (
              <ActionLink variant="secondary" href="/saved">
                Saved homes & searches
              </ActionLink>
            )}
            {moderatorMembership && (
              <ActionLink variant="secondary" href="/moderation">
