    ...
    <main className="property-detail-page">
      <header className="property-detail-topbar">
        ...
        <div className="property-detail-nav">
          <Link className="text-link" href="/homes">Back to map</Link>
      ...
      <div className="property-detail-shell">
        <section className="property-detail-hero">
          <div>
            <p className="eyebrow">{label(property.property_type)} · Available now</p>
            <h1>{property.title || "Rental property"}</h1>
            <p className="property-detail-address">{property.address_text || "Exact location shown below"}</p>
          </div>
          <div className="property-detail-price">
            <strong>{property.rent_bdt ? `৳${property.rent_bdt.toLocaleString("en-BD")}` : "Rent on request"}</strong>
            <span>per month</span>
          </div>
        </section>

        <section className="property-gallery">
          {photos.length ? photos.slice(0, 5).map((item, index) => (
            <div className={`property-gallery-item ${index === 0 ? "property-gallery-primary" : ""}`} key={item.id}>
              <Image src={item.signed_url!} alt={`${property.title || "Property"} photo ${index + 1}`} fill sizes={index === 0 ? "(max-width: 900px) 100vw, 60vw" : "(max-width: 900px) 50vw, 20vw"} />
            </div>
          )) : <div className="property-gallery-empty">No property photos are available.</div>}
        </section>
        ...
        <div className="property-detail-layout">
          <div className="property-detail-main">
            <section className="property-detail-section property-summary-grid">
              ...
              <div><strong>{property.size_sqft ? property.size_sqft.toLocaleString("en-BD") : "—"}</strong><span>Sq ft</span></div>
              <div><strong>{property.floor_number ?? "—"}{property.total_floors ? ` / ${property.total_floors}` : ""}</strong><span>Floor</span></div>
            </section>

            <section className="property-detail-section">
              <h2>About this home</h2>
              <p className="property-description">{property.description || "The owner has not added a longer description yet."}</p>
              <dl className="property-facts">
                <div><dt>Property type</dt><dd>{label(property.property_type)}</dd></div>
                <div><dt>Furnishing</dt><dd>{label(property.furnishing)}</dd></div>
                ...
                <div><dt>Gender preference</dt><dd>{label(property.gender_preference)}</dd></div>
              </dl>
            </section>

            <section className="property-detail-section">
              <h2>Tenant compatibility</h2>
              <p className="section-copy">These are the tenant types the owner has explicitly marked as acceptable for this listing.</p>
              <div className="property-tags">{property.tenant_types.map((type) => <span key={type}>{label(type)}</span>)}</div>
            </section>

            <section className="property-detail-section">
              <h2>Amenities & included utilities</h2>
              <div className="property-tag-groups">
                <div><h3>Amenities</h3><div className="property-tags">{property.amenities.length ? property.amenities.map((amenity) => <span key={amenity.slug}>{amenity.name}</span>) : <span>None listed</span>}</div></div>
                <div><h3>Utilities included</h3><div className="property-tags">{property.utilities_included.length ? property.utilities_included.map((utility) => <span key={utility}>{label(utility)}</span>) : <span>None listed</span>}</div></div>
              </div>
            </section>
            ...
            {videos.length > 0 && (
              <section className="property-detail-section">
                ...
                <div className="property-video-grid">{videos.map((item) => <video key={item.id} src={item.signed_url!} controls preload="metadata" />)}</div>
              </section>
            )}

            <section className="property-detail-section">
              <h2>Exact location</h2>
              <p className="section-copy">The owner pinned this exact location when creating the listing.</p>
              <div className="property-map"><iframe title="Exact property location" src={mapUrl} loading="lazy" /></div>
            </section>
          </div>
