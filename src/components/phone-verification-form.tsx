...
type PhoneMessageKey = "errorRateLimited" | "errorInvalidCode" | "errorSmsUnavailable" | "errorFailed" | "errorInvalidPhone" | "errorRequestFirst" | "errorEnterCode";

function friendlyError(message: string): PhoneMessageKey {
  const lower = message.toLowerCase();
  if (lower.includes("rate limit")) return "errorRateLimited";
  if (lower.includes("expired") || lower.includes("invalid") || lower.includes("token")) return "errorInvalidCode";
  if (lower.includes("phone") || lower.includes("provider") || lower.includes("sms")) return "errorSmsUnavailable";
  return "errorFailed";
}
...
export function PhoneVerificationForm({ currentPhone, isVerified }: { currentPhone: string | null; isVerified: boolean }) {
  ...
  async function verifyCode(event: FormEvent) {
    ...
    const { error } = await supabase.auth.verifyOtp({ phone: phoneBeingVerified, token, type: "phone_change" });
    if (error) {
      setMessageKey(friendlyError(error.message));
      setBusy(false);
      return;
  ...
  const successPhone = maskPhone(verifiedPhone ?? currentPhone, copy.fallbackPhone);
    ...
    <div className="phone-verification-flow">
      <ol className="phone-verification-progress" aria-label={copy.progressAria}>
        <li className={stage === "number" ? "is-current" : stage === "code" || stage === "verified" ? "is-complete" : ""} aria-current={stage === "number" ? "step" : undefined}>
          <span>{formatNumber(1, { useGrouping: false })}</span><div><strong>{copy.numberStep}</strong><small>{copy.numberStepDescription}</small></div>
        </li>
        <li className={stage === "code" ? "is-current" : stage === "verified" ? "is-complete" : ""} aria-current={stage === "code" ? "step" : undefined}>
          <span>{formatNumber(2, { useGrouping: false })}</span><div><strong>{copy.codeStep}</strong><small>{copy.codeStepDescription}</small></div>
        </li>
        <li className={stage === "verified" ? "is-current is-complete" : ""} aria-current={stage === "verified" ? "step" : undefined}>
          <span>{formatNumber(3, { useGrouping: false })}</span><div><strong>{copy.verifiedStep}</strong><small>{copy.verifiedStepDescription}</small></div>
        </li>
      </ol>
      ...
      <section className="phone-verification-stage" aria-live="polite">
        {stage === "number" && (
          <div className="phone-verification-stage-grid">
            <div className="phone-verification-stage-intro">
              ...
              <h2>{isVerified ? copy.changeNumberTitle : copy.addNumberTitle}</h2>
              <p>{copy.numberDescription}</p>
              <div className="phone-verification-privacy-note"><LockKeyhole size={17} aria-hidden="true" /><span>{copy.privacyNote}</span></div>
            </div>
            ...
            <form className="phone-verification-form-card" onSubmit={requestCode}>
              ...
              <div className="bd-phone-field phone-verification-number-field">
                ...
                <input id="verification-phone-number" type="tel" inputMode="numeric" autoComplete="tel-national" placeholder="1712345678" value={phoneInput} onChange={(event) => setPhoneInput(bangladeshPhoneSubscriberDigits(event.target.value))} maxLength={14} aria-describedby="verification-phone-guidance" disabled={busy} />
              </div>
              <p className="form-hint" id="verification-phone-guidance">{copy.mobileGuidance}</p>
              {isVerified && <p className="phone-verification-existing-note">{copy.existingNote}</p>}
              <button className="primary-button" type="submit" disabled={busy}>{busy ? copy.sendingCode : copy.sendCode}</button>
            </form>
          </div>
        )}
        ...
        {stage === "code" && pendingPhone && (
          <div className="phone-verification-code-stage">
            ...
            <p>{copy.sentCodePrefix} <strong>{maskPhone(pendingPhone, copy.fallbackPhone)}</strong>।</p>
            <form className="phone-verification-code-form" onSubmit={verifyCode}>
              <label htmlFor="verification-code">{copy.codeLabel}</label>
              <input id="verification-code" inputMode="numeric" autoComplete="one-time-code" maxLength={6} placeholder="123456" value={token} onChange={(event) => setToken(event.target.value.replace(/\D/g, "").slice(0, 6))} disabled={busy} autoFocus />
              <button className="primary-button" type="submit" disabled={busy || token.length !== 6}>{busy ? copy.verifying : copy.verifyPhone}</button>
            </form>
            <div className="phone-verification-code-actions">
              <button className="text-button" type="button" onClick={editNumber} disabled={busy}>{copy.editNumber}</button>
              <button className="text-button" type="button" onClick={() => void requestCode()} disabled={busy || resendSeconds > 0}>{resendSeconds > 0 ? interpolate(copy.resendIn, { seconds: formatNumber(resendSeconds, { useGrouping: false }) }) : copy.resendCode}</button>
            </div>
          </div>
        )}
        ...
        {stage === "verified" && (
          <div className="phone-verification-success-stage">
            ...
            <p>{interpolate(copy.successDescription, { phone: successPhone })}</p>
            <div className="phone-verification-success-badge"><CheckCircle2 size={16} aria-hidden="true" /> {copy.successBadge}</div>
            <p className="phone-verification-disclaimer">{copy.disclaimer}</p>
            <button className="secondary-button" type="button" onClick={editNumber}>{copy.changeVerifiedNumber}</button>
          </div>
        )}

        {messageKey && <div className="auth-message phone-verification-message" role="alert">{copy[messageKey]}</div>}
      </section>
    </div>
  );
}
