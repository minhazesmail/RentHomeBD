    ...
    <>
      <section className={styles.gallery} aria-label="Property photos and videos">
        <div className={styles.grid}>
          {previewItems.map((item, index) => (
            <button className={`${styles.tile} ${index === 0 ? styles.primary : ""}`} type="button" key={item.id} onClick={(event) => openGallery(index, event.currentTarget)} aria-label={`Open ${item.media_type === "photo" ? "photo" : "video"} ${index + 1} of ${orderedMedia.length}`}>
              {item.media_type === "photo" ? (
                <Image src={item.signed_url} alt={`${propertyTitle} photo ${index + 1}`} fill sizes={index === 0 ? "(max-width: 900px) 100vw, 60vw" : "(max-width: 900px) 50vw, 20vw"} onError={() => recoverMedia(item)} />
              ) : (
                <><video src={item.signed_url} muted preload="metadata" playsInline onError={() => recoverMedia(item)} /><span className={styles.playBadge}><Play size={18} fill="currentColor" aria-hidden="true" />Video</span></>
              )}
            </button>
          ))}
        </div>
        <button className={styles.countButton} type="button" onClick={(event) => openGallery(0, event.currentTarget)}><Camera size={15} aria-hidden="true" /><span>{photoCount} photo{photoCount === 1 ? "" : "s"}{videoCount ? ` · ${videoCount} video${videoCount === 1 ? "" : "s"}` : ""}</span></button>
      </section>
      ...
      {activeItem && activeIndex !== null && (
        <div ref={lightboxRef} className={styles.lightbox} role="dialog" aria-modal="true" aria-label={`${propertyTitle} media viewer`} onMouseDown={(event) => { if (event.target === event.currentTarget) closeGallery(); }}>
          <div className={styles.lightboxHeader}><span>{activeIndex + 1} / {orderedMedia.length} · {activeItem.media_type === "photo" ? "Photo" : "Video"}</span><button ref={closeButtonRef} className={styles.iconButton} type="button" onClick={closeGallery} aria-label="Close media viewer"><X aria-hidden="true" /></button></div>
          <div className={styles.stage}>
            ...
            <div className={styles.activeMedia}>
              {activeItem.media_type === "photo" ? <Image src={activeItem.signed_url} alt={`${propertyTitle} photo ${activeIndex + 1}`} fill sizes="100vw" priority onError={() => recoverMedia(activeItem)} /> : <video key={activeItem.id} src={activeItem.signed_url} controls autoPlay playsInline preload="metadata" onError={() => recoverMedia(activeItem)}>Your browser does not support property video playback.</video>}
            </div>
