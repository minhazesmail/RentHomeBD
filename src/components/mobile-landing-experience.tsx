      ...
      <header className={styles.nav}>
        <div className={styles.navTop}>
          ...
          <div className={styles.navActions}>
            ...
            <Link className={styles.listButton} href={LIST_PROPERTY_HREF}>{dictionary.navigation.listProperty}</Link>
          </div>
        </div>
        <nav className={styles.navLinks} aria-label={dictionary.navigation.primaryNavigationAria}>
          <Link href="/homes">{dictionary.navigation.findOnMap}</Link>
          <a href="#how-heading">{dictionary.navigation.howItWorks}</a>
          <Link href="/about">{dictionary.navigation.about}</Link>
        </nav>
      </header>

      <section className={styles.hero} aria-labelledby="mobile-concept-heading">
        <div className={styles.kicker}><span aria-hidden="true" />{copy.kicker}</div>

        <div className={styles.heroStage}>
          <div className={styles.heroCopy}>
            <h1 id="mobile-concept-heading">
              <span>{copy.mobileTitleLead}</span>
              <em>{copy.mobileTitleAccent}</em>
            </h1>
        ...
        <div className={styles.signals} aria-label={copy.mobileSignalsAria}>
          <div>
            <span className={styles.signalIcon} aria-hidden="true"><MapPin /></span>
            <strong>{copy.mobileDhakaFocused}</strong>
          </div>
          <i aria-hidden="true" />
          <div>
            <span className={styles.signalIcon} aria-hidden="true"><ShieldCheck /></span>
            <strong>{copy.mobileTenantAware}</strong>
          </div>
        </div>
        ...
        <form className={styles.searchCard} action="/homes" method="get" role="search" onSubmit={handleSubmit}>
          <div className={styles.searchHeading}>
            <div>
              ...
              <small>{copy.mobileSearchSubtitle}</small>
            </div>
          ...
          <label className={styles.areaField}>
            ...
            <input
              ...
              list="mobile-landing-location-options"
              placeholder={copy.mobileLocationPlaceholder}
              autoComplete="off"
              required
              aria-describedby="mobile-landing-search-help"
            />
            <datalist id="mobile-landing-location-options">
          ...
          <div className={styles.filterRow} aria-label={copy.mobileFilterAria}>
            <button type="button" data-mobile-filter="budget" onClick={(event) => openFilter("budget", event.currentTarget)}>
              <Banknote aria-hidden="true" /><span>{budgetLabel}</span>
            </button>
            <button type="button" data-mobile-filter="tenant" className={tenant ? styles.filterSelected : undefined} onClick={(event) => openFilter("tenant", event.currentTarget)}>
              <Users aria-hidden="true" /><span>{tenantLabel}</span>
            </button>
            <button type="button" onClick={(event) => openFilter("bedrooms", event.currentTarget)}>
              <BedDouble aria-hidden="true" /><span>{bedroomLabel}</span>
            </button>
            <button type="button" onClick={(event) => openFilter("more", event.currentTarget)}>
              <SlidersHorizontal aria-hidden="true" /><span>{moreLabel}</span>
            </button>
          </div>
          ...
          <p className={styles.searchHelp} id="mobile-landing-search-help">{copy.searchHelp}</p>
